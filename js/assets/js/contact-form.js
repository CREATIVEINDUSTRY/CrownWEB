(function () {
  ...
  $(document).ready(function() {
   /* if ($("#phone").intlTelInput) {
      $("#phone").intlTelInput({validationScript: "./js/assets/vender/intl-tel-input/js/isValidNumber.js"});
    ...
    $("#feedbackSubmit").click(function() {
      ...
      $.ajax({
        type: "POST",
        url: "./img/library/sendmail.php",
        data: $("#feedbackForm").serialize(),
        success: function(data) {
          contactFormUtils.addAjaxMessage(data.message, false);
          contactFormUtils.clearForm();
          //get new Captcha on success
          $('#captcha').attr('src', './img/library/vender/securimage/securimage_show.php?' + Math.random());
        },
    ...
    $('#feedbackForm input, #feedbackForm textarea').change(function () {
      var checkBox = $(this).siblings('span.input-group-addon').children('.fa-check-square-o');
      if ($(this).val()) {
